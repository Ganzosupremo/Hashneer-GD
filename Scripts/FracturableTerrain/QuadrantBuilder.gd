...
@onready var _terrain_block_template: PackedScene = preload("res://Scenes/MiningGameModeTerrainBlock/TerrainBlock.tscn")
...
## [param other_body]: The FracturableStaticBody2D that is being fractured.[br]
## [param bullet_launch_velocity]: The launch velocity of the bullet.[br]
## [param bullet_damage]: The damage dealt by the bullet.[br]
...
func _apply_vein_bonus(vein_center: Vector2i, ore_type: OreDetails.OreType, vein_dict: Dictionary) -> void:
	for x in range(-1, 2):
		for y in range(-1, 2):
			if x == 0 and y == 0:
				continue  # Skip center
						
			var neighbor = vein_center + Vector2i(x, y)
			...
			# Check if cell would be in shape
			if not _is_cell_in_shape(neighbor):
				continue
						
			# 40% chance to propagate
			if _rng.randf() < 0.4 and not vein_dict.has(neighbor):
...
func _get_ore_resource(ore_type: OreDetails.OreType) -> OreDetails:
	if ores_dictionary.has(ore_type):
		...
		return ores_dictionary[OreDetails.OreType.DIRT]  # Fallback
...
func _spawn_staticbody(shape_info : Dictionary, color : Color, texture_info : Dictionary, ore_info : Dictionary = {}) -> void:
	var instance_staticbody: TerrainBlock = _terrain_block_template.instantiate()
...
func _create_terrain_block_args(ore_type: OreDetails.OreType, ore_data: OreDetails, depth_layer: int, health: float) -> TerrainBlock.TerrainBlockArgs:
