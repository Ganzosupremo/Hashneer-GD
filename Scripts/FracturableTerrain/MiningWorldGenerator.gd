...
##
## This class handles terrain generation including:
...
enum TerrainLayer { AIR, DIRT, STONE, DEEP_STONE, BEDROCK, SHOP_WALL }

var quadrant_size: Vector2i = Vector2i.ZERO
...
## Shop chamber configuration
var shop_chamber_enabled: bool = true
var shop_chamber_width: int = 12  ## Width in grid cells (interior)
var shop_chamber_height: int = 6  ## Height in grid cells (interior)
var shop_chamber_depth_ratio: float = 0.25  ## How deep into stone layer (0.0 = just below dirt, 1.0 = near bedrock)
var shop_chamber_wall_thickness: int = 1  ## Thickness of indestructible walls

var _shop_chamber_bounds: Rect2i = Rect2i()  ## Grid cell bounds of chamber interior
...
func _carve_shop_chamber() -> void:
	if not shop_chamber_enabled:
		_shop_chamber_bounds = Rect2i()
		return
		
	var avg_surface: int = 0
	for height in _surface_heights:
		avg_surface += height
	avg_surface = avg_surface / _surface_heights.size()
		
	var stone_start_y: int = avg_surface + dirt_layer_thickness
	var stone_end_y: int = grid_size.y - 2  ## Leave room for bedrock
	var stone_depth: int = stone_end_y - stone_start_y
		
	var chamber_y: int = stone_start_y + int(stone_depth * shop_chamber_depth_ratio)
	var chamber_x: int = (grid_size.x - shop_chamber_width) / 2
		
	chamber_x = clampi(chamber_x, shop_chamber_wall_thickness + 1, grid_size.x - shop_chamber_width - shop_chamber_wall_thickness - 1)
	chamber_y = clampi(chamber_y, stone_start_y, stone_end_y - shop_chamber_height - shop_chamber_wall_thickness)
		
	_shop_chamber_bounds = Rect2i(chamber_x, chamber_y, shop_chamber_width, shop_chamber_height)
		
	var wall_left: int = chamber_x - shop_chamber_wall_thickness
	var wall_right: int = chamber_x + shop_chamber_width + shop_chamber_wall_thickness - 1
	var wall_top: int = chamber_y - shop_chamber_wall_thickness
	var wall_bottom: int = chamber_y + shop_chamber_height + shop_chamber_wall_thickness - 1
		
	for x in range(wall_left, wall_right + 1):
		for y in range(wall_top, wall_bottom + 1):
			var cell: Vector2i = Vector2i(x, y)
						
			if x < 1 or x >= grid_size.x - 1 or y < 0 or y >= grid_size.y - 1:
				continue
						
			var is_interior: bool = (
				x >= chamber_x and x < chamber_x + shop_chamber_width and
				y >= chamber_y and y < chamber_y + shop_chamber_height
			)
						
			if is_interior:
				_terrain_map[cell] = TerrainLayer.AIR
			else:
				_terrain_map[cell] = TerrainLayer.SHOP_WALL
		
	print("MiningWorldGenerator: Shop chamber carved at grid position (%d, %d) size %dx%d" % [chamber_x, chamber_y, shop_chamber_width, shop_chamber_height])
...
func _carve_caves() -> void:
	for x in range(1, grid_size.x - 1):
		...
		for y in range(surface_y + dirt_layer_thickness, grid_size.y - 1):
			...
			if layer == TerrainLayer.BEDROCK or layer == TerrainLayer.SHOP_WALL:
				continue
...
func _spawn_terrain_blocks() -> void:
	...
	for x in range(grid_size.x):
		for y in range(grid_size.y):
			...
			var is_indestructible: bool = is_border or layer == TerrainLayer.BEDROCK or layer == TerrainLayer.SHOP_WALL
...
func _spawn_border_block(cell: Vector2i, pos: Vector2, layer: TerrainLayer) -> void:
	...
	match layer:
		...
		TerrainLayer.SHOP_WALL:
			block.self_modulate = Color(0.25, 0.22, 0.18, 1.0)  ## Dark bronze/copper for shop walls
		_:
...
func _setup_world_bounds() -> void:
	...
	if shop_layer:
		var chamber_world_bounds: Rect2 = get_shop_chamber_world_bounds()
		if chamber_world_bounds.size != Vector2.ZERO:
			shop_layer.initialize_in_chamber(chamber_world_bounds)
		else:
			shop_layer.initialize(terrain_width)
				
	if digital_ambiance:
		digital_ambiance.set_world_bounds(_map_bounds)
...
func get_shop_chamber_bounds() -> Rect2i:
	return _shop_chamber_bounds

func get_shop_chamber_world_bounds() -> Rect2:
	if _shop_chamber_bounds.size == Vector2i.ZERO:
		return Rect2()
	return Rect2(
		Vector2(_shop_chamber_bounds.position.x * quadrant_size.x, _shop_chamber_bounds.position.y * quadrant_size.y),
		Vector2(_shop_chamber_bounds.size.x * quadrant_size.x, _shop_chamber_bounds.size.y * quadrant_size.y)
	)
