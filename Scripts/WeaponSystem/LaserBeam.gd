...
func spawn(pos: Vector2, origin_transform: Transform2D, launch_vector: Vector2, lifetime: float, fracture_mgr: FractureManager, ammo_data: AmmoDetails) -> void:
        _ammo_details = ammo_data
        _damage_cooldown = ammo_data.laser_damage_cooldown
        _fracture_manager = fracture_mgr
...
func _apply_damage(body: Node2D, hit_pos: Vector2) -> void:
        ...
        if body is FracturableStaticBody2D and body is not BlockCore and _fracture_manager:
                _fracture_manager.call_deferred("fracture_on_collision", hit_pos, body, _launch_velocity, damage_to_deal, _ammo_details.bullet_speed)
