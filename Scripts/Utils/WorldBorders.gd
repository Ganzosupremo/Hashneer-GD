...
var _shop_chamber_bounds: Rect2 = Rect2()

## Set the shop chamber bounds (for integrated underground chamber)
func set_shop_chamber_bounds(bounds: Rect2) -> void:
        _shop_chamber_bounds = bounds

## Get the shop layer bounds as a Rect2.[br]
## Returns chamber bounds if set, otherwise legacy floating layer bounds
## [return: Rect2] The shop layer/chamber bounds
func get_shop_layer_bounds() -> Rect2:
        if _shop_chamber_bounds.size != Vector2.ZERO:
